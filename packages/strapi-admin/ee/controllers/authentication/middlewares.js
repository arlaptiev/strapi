...
const authenticate = async (ctx, next) => {
  ...
  return passport.authenticate(provider, null, async (error, profile) => {
    if (error || !profile) {
      ...
      return ctx.redirect(constants.PROVIDER_REDIRECT_ERROR);
    }
    ...
    // If the the configuration is incorrect or the autoRegister feature is disabled, redirect with an error
    if (!providers.autoRegister || !providers.defaultRole) {
      return ctx.redirect(constants.PROVIDER_REDIRECT_ERROR);
    }
...
const redirectWithAuth = ctx => {
  ...
  ctx.redirect(constants.PROVIDER_REDIRECT_SUCCESS);
};
